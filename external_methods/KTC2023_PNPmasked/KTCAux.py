...
def simulateConductivity(Meshsim, inclusiontypes):
    ...
    if inclusiontypes == 2:
        ...
        delta_sigma[ind] = abs(contrast)
    sigma2 = sigma + delta_sigma
    for i in range(Meshsim.g.shape[0]):
        if sigma2[i]==1:
            label[i]=1
      
            
    return sigma, delta_sigma, sigma2, label
