"""Experiment runner â€” loops over levels, samples, and methods from config."""
...
class BatchRunner:
    ...
    def _run_one(self, method: str, level: int, sample: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            console.print(f"[yellow]data_plugin '{plugin_name}' not registered â€” falling back to MockDataPlugin.[/yellow]")
        ...
        try:
            ...
            console.print(f"[yellow]Skipping level={level} sample={sample} â€” file not found in '{dataset_root}'.[/yellow]")
            return None
        except ValueError as exc:
            console.print(f"[red]Skipping level={level} sample={sample} â€” validation error: {exc}[/red]")
            return None

        try:
            method_plugin = registry_get(method)()
        except KeyError:
            console.print(f"[red]Method '{method}' not registered â€” skipping.[/red]")
            return None
    ...
    def _save(self, results: list[dict[str, Any]]) -> None:
        ...
        # Nested structure: method â†’ level â†’ sample â†’ metrics
        nested: dict[str, Any] = {}
        for r in results:
            m = r["method"]
            lv = str(r["level"])
