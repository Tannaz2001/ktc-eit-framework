...
## `ktc2023_abc2` — Integration Record

**Source**: https://github.com/robert-abc/KTC2023-ABC2  
**Algorithm**: Deep Image Prior (DIP) + Total Variation regularization. Optimises a randomly-initialised convolutional network to fit EIT voltage measurements; interpolates to 256×256 and segments into {0, 1, 2} via three-class Otsu thresholding.

### What was needed

The original `main.py` saves PNG files, not `.mat` files — incompatible with the framework output contract. A wrapper `run_method.py` was created that:
- Accepts the same CLI arguments (`input_path output_path category`)
- Imports `utils.solver.solve()` directly
- Rounds the output to integer labels {0, 1, 2}
- Saves `{idx}.mat` with a `reconstruction` field

No pre-trained weights are needed. The DIP method optimises from scratch for each sample (~3000 iterations). **Expect slow runtime on CPU: 10–30 minutes per sample.**

### Dependencies

```
pip install torchmetrics Pillow    # install on whichever Python has torch
```

### Benchmark config

```
configs/ktc_abc2.yaml
```

---

## `ktc2023_e2e` — Integration Record

**Source**: https://github.com/lucala00/KTC2023_E2E  
**Algorithm**: End-to-end UNet with a learnable first linear layer initialised from a Tikhonov reconstruction. A separate model is trained per difficulty level. Maps voltage measurements directly to a 256×256 three-class segmentation.

### What was needed

The original `main.py` defines `main(inputFolder, outputFolder, difficulty)` with no `__main__` / argparse block, and has two issues when run as a CLI subprocess:

1. **No entry point**: `main.py` is not directly callable from the command line.
2. **`torch.load` without `map_location`**: Models saved on GPU would fail to load on CPU-only machines.

A wrapper `run_method.py` was created that:
- Sets `matplotlib.use("Agg")` before any imports (prevents `plt.show()` from blocking)
- Monkey-patches `torch.load` to default `map_location` to CPU
- Parses CLI args and calls `main.main()`

**Model weights are bundled** (847 MB total, downloaded via git LFS):
```
external_methods/KTC2023_E2E/models/difficulty_{1-7}_learnedlinear.pth.tar
```
...
```
configs/ktc_e2e.yaml
```

---

## `ktc2023_pnpmasked` — Integration Record

**Source**: https://github.com/lucala00/KTC2023_PNPmasked  
**Algorithm**: Plug-and-Play Proximal Gauss-Newton with a pre-trained Graph U-Net denoiser as the proximal operator. Post-processing applies synthetic boundary masks (per difficulty level) to suppress electrode-gap artefacts.

### What was needed

The original `main.py` defines `main(inputFolder, outputFolder, categoryNbr)` with no `__main__` / argparse block, and uses relative paths for all assets.

A wrapper `run_method.py` was created that:
- Sets `matplotlib.use("Agg")` before any imports
- Changes CWD to the method directory (so `Mesh_sparse.mat`, `./weights_denoiser.pth`, and `Mask_Synthetic/` resolve correctly)
- Parses CLI args and calls `main.main()`

**All assets are bundled**:
```
external_methods/KTC2023_PNPmasked/
  weights_denoiser.pth   ← 163 KB Graph U-Net weights
  Mesh_sparse.mat        ← FEM mesh
  Mask_Synthetic/        ← boundary masks for all levels
```
...
For full GPU-accelerated scatter/sparse operations (optional, CPU works):
```
pip install torch_scatter torch_sparse -f https://data.pyg.org/whl/torch-2.13.0+cpu.html
```

### Benchmark config

```
configs/ktc_pnpmasked.yaml
```
